...
def log_probs_from_logits_flash_attn(logits: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
    ...
    ## GPT5
    # try:
    #     # Use safer backward to avoid potential illegal memory accesses from in-place backward
    #     output = cross_entropy_loss(logits, labels, inplace_backward=False)
    ...
    #     return -losses
    # except Exception:
    #     # On any kernel failure, fall back to the stable PyTorch implementation
